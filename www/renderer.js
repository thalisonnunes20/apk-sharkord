document.getElementById('setup-form').addEventListener('submit', (e) => {
  ...
  try {
    ...
    // Send to main process (Substituído para Android)
    // Atualiza histórico de servidores recentes (max 2)
    let recents = JSON.parse(localStorage.getItem('recentServers') || '[]');
    recents = recents.filter(item => item !== url.href);
    recents.unshift(url.href);
    if (recents.length > 2) recents = recents.slice(0, 2);
    localStorage.setItem('recentServers', JSON.stringify(recents));

    // Redireciona a WebView para o servidor Sharkord
...
// Como estamos no navegador do celular, falhas de conexão serão exibidas pelo próprio navegador nativo
// Não precisamos do onConnectionError do Electron.

// Carrega os servidores recentes ao iniciar a página (Android via localStorage)
document.addEventListener('DOMContentLoaded', () => {
  try {
    const recents = JSON.parse(localStorage.getItem('recentServers') || '[]');
    if (recents && recents.length > 0) {
      
      // Auto-reconectar ao último servidor
      document.getElementById('server-url').value = recents[0];
